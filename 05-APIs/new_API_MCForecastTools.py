...
class MCSimulation:
    ...
    def __init__(self, portfolio_data, weights="", num_simulation=1000, num_trading_days=252):
        ...
        if weights == "":
            num_stocks = len(portfolio_data.columns.get_level_values(1).unique())
        ...
        if not "daily_return" in portfolio_data.columns.get_level_values(0).unique():
            close_df = portfolio_data['close'].pct_change().dropna()
#            print(close_df.head())
            tickers = portfolio_data.columns.get_level_values(1).unique()
            column_names = [("daily_return",x) for x in tickers]
            close_df.columns = pd.MultiIndex.from_tuples(column_names)
            portfolio_data = portfolio_data.merge(close_df,left_index=True,right_index=True)
...
#        last_prices = self.portfolio_data.xs('close',level=1,axis=1)[-1:].values.tolist()[0]
        last_prices = self.portfolio_data['close'][-1:].values.tolist()[0]
...
#        daily_returns = self.portfolio_data.xs('daily_return',level=1,axis=1)
        daily_returns = self.portfolio_data['daily_return']
